...
The first derivative of the log-likelihood, $\lik'(\th)$, is important enough to have its own name: the *score function*.
...
$$\left\{ \mathfrak{I}_{ij} \eqdef {Cov}\left( {\ell'}_{i},{\ell'}_{j} \right) = E[ \ell_{i}'\ell_{j}' ] - E[ {\ell'}_{i} ] E[ {\ell'}_{j} ] \right\}$$
...
### Gradient ascent {#sec-newton-raphson}
...
\begin{aligned}
\widehat{\theta} 
&\leftarrow \widehat{\theta} + [ I\left( \widehat{\theta},y \right) ]^{- 1}\ell'\left( y,\widehat{\theta} \right)\\
&= \widehat{\theta} - [ \ell''\left( y,\widehat{\theta} \right) ]^{- 1}\ell'\left( y,\widehat{\theta} \right)
\end{aligned}
$$
