...
The exposure magnitude, $T$, is *similar* to a covariate in linear or logistic regression. However, there is an important difference: in count regression, **there is no intercept corresponding to $\Expp[Y|T=0]$**. In other words, this model assumes that if there is no exposure, there can't be any events.
