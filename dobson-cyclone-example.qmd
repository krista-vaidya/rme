...
Then we could start with some initial guess for $\hat \lambda$, 
such as $\hat\lambda = 1$.

Then we could use the Newton-Raphson algorithm.

In @exr-hessian, We found that the score function was:

$$
\ell'( \lambda; \vec x ) = \paren{\frac{1}{\lambda} n \xbar} - n
$$

and the Hessian was:

$$
\ell''( \lambda; \vec x ) = -\frac{1}{\lambda^2} n \bar x
$$

In the dataset, $\xbar = `r xbar`$.
