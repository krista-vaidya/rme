...
$$\logit(\pi) \eqdef \log{\frac{\pi}{1-\pi}}$$
...
$$
\ba
\ell(\pi, \vec y) &= \sumin y_i \log{\pi_i} + (1 - y_i) \log{1-\pi_i}
\\                &= \sumin y_i \red{\log{\pi_i}} + \log{1-\pi_i} - y_i \cd \blue{\log{1-\pi_i}}
\\                &= \sumin y_i (\red{\log{\pi_i}} - \blue{\log{1-\pi_i}}) + \log{1-\pi_i}
\\                &= \sumin y_i (\log{\frac{\red{\pi_i}}{\blue{1-\pi_i}}}) + \log{1-\pi_i}
\\                &= \sumin y_i (\logit(\pi_i)) + \log{1-\pi_i}
\ea
...
- $Y_i|\vX_i \simiid \Ber(\pi(X_i))$
- $\logitf{\pi(\vx)} = \vx'\beta$
...
\ell'(\vb) 
   &= \deriv{\beta}\ell(\vb)
\\ &= \deriv{\beta}\sumin y_i \paren{\logitf{\pi_i}} + \log{1-\pi_i}
\\ &= \sumin \deriv{\beta}\cb{y_i \paren{\vx_i'\beta} + \log{1-\pi_i}}
\\ &= \sumin \cb{y_i \deriv{\beta}\paren{\vx_i'\beta} + \deriv{\beta}\log{1-\pi_i}}
\\ &= \sumin \cb{\vx_i y_i + \deriv{\beta}\log{1-\expit(\vx_i'\beta)}}
\\ &= \sumin \cb{\vx_i y_i + \deriv{\beta}\log{\inv{1+\exp{\vx_i'\beta}}}}
\\ &= \sumin \cb{\vx_i y_i - \deriv{\beta}\log{1+\exp{\vx_i'\beta}}}
\ea
$$
...
\ba
\ell'(\vb) 
   &= \sumin \cb{\vx_i y_i - \vx_i \expitf{\vx_i'\beta} }
\\ &= \sumin \vx_i \cb{ y_i - \expitf{\vx_i'\beta}}
\\ &= \sumin \vx_i \cb{ y_i - \inv{1+\expf{-\vx_i'\beta}}}
\ea
$$
---
...
Instead, we have to use a variant of the [Newton-Raphson method](intro-MLEs.qmd#sec-newton-raphson) (c.f., @dobson4e, Chapter 4).
