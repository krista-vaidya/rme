...
Consider two covariate patterns, $\vx$ and $\vx^*$.

The odds ratio comparing these covariate patterns is:

$$
\omega(\vx,\vx^*) = \exp{(\vx-\vx^*)\'\vb}
$$
...
\ba
\omega(\vx,\vx^*) 
&= \frac {\odds(Y=1|\vX = \vx)} {\odds(Y=1|\vX = \vx^*)}
\\ &= \frac{\exp{\vx\'\vb}}{\exp{{\vx^*}\'\vb}}
\\ &= \exp{\vx\'\vb - {\vx^*}\' \vb}
\\ &= \exp{(\vx\' - {\vx^*}\') \vb}
\\ &= \exp{{(\vx - \vx^*)}\' \vb}
\ea
...
#| fig-cap: "Residuals vs Fitted plot for CHD models (@tbl-chd-model-coef-ests)"
#| layout-ncol: 2
#| fig-subcap: 
#| - "Logistic link"
#| - "Identity link"
