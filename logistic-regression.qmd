...
\ba
\odds'(\pi) 
   &= \deriv{\pi}{\frac{\pi}{1-\pi}}
\\ &=  \frac{\deriv{\pi}\pi}{1-\pi} - \frac{\pi}{\sqf{1-\pi}} \deriv{{1-\pi}}
\\ &=  \frac{1}{1-\pi} - \frac{\pi}{\sqf{1-\pi}} \deriv{{1-\pi}}
\ea
$$
...
The **expit** function (@fig-expit-plot) of a log-odds $\eta$, also known as the **inverse-logit** or **logistic** function, is the inverse-odds of the exponential of $\eta$:

$$\expit(\eta) \eqdef \oddsinvf{\expf{\eta}}$$
...
$$1-\oddsinvf{\omega}) = \frac{1}{1+\omega}$$
:::
...
$$
\ba
1 - \oddsinvf{\omega}) &= 1 - \frac{\omega}{1 + \omega}
\\ &= \frac{\red{1+\omega}}{1 + \omega} - \frac{\blue{\omega}}{1 + \omega}
\\ &= \frac{\red{(1+\omega)} - \blue{\omega}}{1 + \omega}
...
$$
\ba
1 - \expitf{\eta} &= 1 - \oddsinvf{\expf{\eta}}
\\ &= \frac{1}{1+\expf{\eta}}
\\ &= \inv{1+\exp{\eta}}
\ea
$$
...
\red{\ell_i(\pi(\vx_i))}
   &= y_i \log{\pi_i} + (1 - y_i) \log{1-\pi_i}
\\ &= y_i \log{\pi_i} + (1 \cd \log{1-\pi_i} - y_i \cd \log{1-\pi_i})
\\ &= y_i \log{\pi_i} + (\log{1-\pi_i} - y_i \cd \log{1-\pi_i})
\\ &= y_i \log{\red{\pi_i}} + \log{1-\pi_i} - y_i \cd \log{\blue{1-\pi_i}}
\\ &= y_i \log{\red{\pi_i}} - y_i \cd \log{\blue{1-\pi_i}} + \log{1-\pi_i}
\\ &= (y_i \log{\red{\pi_i}} - y_i \cd \log{\blue{1-\pi_i}}) + \log{1-\pi_i}
\\ &= y_i (\log{\red{\pi_i}} - \log{\blue{1-\pi_i}}) + \log{1-\pi_i}
\\ &= y_i \paren{\log{\frac{\red{\pi_i}}{\blue{1-\pi_i}}}} + \log{1-\pi_i}
\\ &= y_i (\logit(\pi_i)) + \log{1-\pi_i}
\ea
...
$$
\llik_i'(\vb) = \vx_i y_i - \vx_i \expitf{\vx_i'\beta} 
$$
...
$$
\llik'(\vb) = \sumin \cb{\vx_i y_i - \vx_i \expitf{\vx_i'\beta} }
$$

Setting $\ell'(\vb) = 0$ gives us:


$$
\ba
\sumin \vx_i y_i
   &= \sumin \cb{ \vx_i \expitf{\vx_i'\beta}} 
\ea
$${#eq-score-logistic}
...
See [Hua Zhou]'s [notes](https://ucla-biostat-200c-2020spring.github.io/slides/04-binomial/binomial.html#:~:text=0.05%20%27.%27%200.1%20%27%20%27%201-,Quasi%2Dbinomial,-Another%20way%20to)
